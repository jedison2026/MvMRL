...
def load_checkpoint(cfg, model, optimizer, lr_scheduler, logger):
    ...
    if not 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:  ## cfg.EVAL_MODE and （这里改了！！！！）
...
# ## 添加 precision和recall: for classification
# def precision_recall(targets, preds):
#     precision, recall, _ = precision_recall_curve(targets, preds)
#     return precision, recall

# ## 添加 accuracy: for regression
# def evs(targets, preds):
#     return explained_variance_score(targets, preds)
...
def get_metric_func(metric):
    ...
    # ## 添加 accuracy: for classification
    # if metric == 'acc':
    #     return accuracy
