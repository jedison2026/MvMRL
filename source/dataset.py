...
from utils import get_task_names  ## source.
...
seq_dict_smi_len = len(seq_dict_smi)
max_seq_smi_len = 100
def seq_smi(smile):
    x = np.zeros(max_seq_smi_len)
...
# ---------------------------------
# def mol_to_fp(mol):
#     ## 添加部分，用于添加混合指纹（这里获取的应该是一个分子的混合指纹）
#     fp = []
...
#     fp_pubcfp = GetPubChemFPs(mol)  ## 自己创建的指纹
#     fp.extend(fp_maccs)
...
# data and dataset
# ---------------------------------
## 这里应该是构建DataFram,在默认的x, edge_attr, edge_index等属性又添加三个属性(猜的)
# class MolData(Data):
...
class MolDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for i, smi in enumerate(tqdm(smilesList)):
            ...
            if data is not None:
                ...
                data.smil2vec = torch.LongTensor([smil2vec])
    ...
    def mol2graph(self, mol):
        ...
        ## mol_to_fps = mol_to_fp(mol)  ## fp部分
        ## fra_edge_index, fra_edge_attr, cluster_index = bond_break(mol)   ## fra部分
        data = Data(
            x=torch.FloatTensor(node_attr),
            edge_index=torch.LongTensor(edge_index).t(),
            edge_attr=torch.FloatTensor(edge_attr),
            ## fp=torch.FloatTensor([mol_to_fps]),  ## fp部分
...
def load_dataset_random(path, dataset, seed, task_type, tasks=None, logger=None):
    ...
    ## del pyg_dataset.data.smiles  ## del是Python中的一个关键字，用于删除变量、列表元素，字典键值对等
...
def load_dataset_noise(path, dataset, seed, task_type, tasks, rate, logger=None):
    ...
    ## 新添加的部分
    if rate is None:
        rate = 0.1  # 给rate一个默认值，或者根据实际情况进行处理
...
def build_loader(cfg, logger):
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=cfg.DATA.BATCH_SIZE, shuffle=True, drop_last=True,)
