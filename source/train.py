...
from model import MvMRL
## from cnn_ablation import MGraphDTA
...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=False,
        default="../configs/delaney/delaney.yaml",  ## 由'../configs/bbbp.yaml'改
        type=str,
    )
...
def train_one_epoch(cfg, model, criterion, trainloader, optimizer, lr_scheduler, device, logger):
    ...
    avg_results = np.nanmean(results)  ## 这里做平均，所以评价指标那里没有取平均
...
def train(cfg, logger):
    ...
    model = MvMRL(out_dim=(len(cfg.DATA.TASK_NAME))*2)  ##, filter_num=32)
    logger.info(model)
    # device mode
    ...
    # step 7: model resuming (if training is interrupted, this will work.)  （添加best_acc_score和best_pre_score两部分）
    best_epoch, best_score = 0, 0 if cfg.DATA.TASK_TYPE == 'classification' else float('inf')
    if cfg.TRAIN.RESUME:
        best_epoch, best_score = load_checkpoint(cfg, model, optimizer, lr_scheduler, logger)   ## 添加best_acc_score, best_pre_score
    ...
    for epoch in range(cfg.TRAIN.START_EPOCH, cfg.TRAIN.MAX_EPOCHS):
        ...
        trn_loss, trn_score= train_one_epoch(cfg, model, criterion, train_loader, optimizer,
                                              lr_scheduler, device, logger)   ## 添加 trn_acc_score, trn_pre_score
        val_loss, val_score = validate(cfg, model, criterion, val_loader, epoch, device, logger)  ## 添加 val_acc_score, val_pre_score
        # Just for observing the testset results during training
        test_loss, test_score= validate(cfg, model, criterion, test_loader, epoch, device, logger)  ## 添加 test_acc_score, test_pre_score
        ...
        if epoch % cfg.SHOW_FREQ == 0 or epoch == cfg.TRAIN.MAX_EPOCHS - 1:
            ...
            logger.info(f'Epoch:{epoch} {cfg.DATA.DATASET} trn_loss:{trn_loss:.5f} '
                        f'trn_{cfg.DATA.METRIC}:{trn_score:.5f} '
                        f'lr:{lr_cur:.5f}')  ## 添加 trn_acc,trn_pre
            logger.info(f'Epoch:{epoch} {cfg.DATA.DATASET} val_loss:{val_loss:.5f} '
                        f'val_{cfg.DATA.METRIC}:{val_score:.5f}'
                        f' lr:{lr_cur:.5f}')  ## 添加 val_acc, val_pre
            logger.info(f'Epoch:{epoch} {cfg.DATA.DATASET} test_loss:{test_loss:.5f} '
                        f'test_{cfg.DATA.METRIC}:{test_score:.5f}'
                        f' lr:{lr_cur:.5f}')  ## 添加 test_acc, test_pre

        # 4: Tensorboard for training visualization.
        loss_dict, acc_dict= {"train_loss": trn_loss}, {f"train_{cfg.DATA.METRIC}": trn_score} ## accuracy_dict, pre_dict以及相应部分
        loss_dict["valid_loss"], acc_dict[f"valid_{cfg.DATA.METRIC}"] = val_loss, val_score  ## 添加accuracy_dict, pre_dict以及相应部分
        ...
        if cfg.DATA.TASK_TYPE == 'classification' and val_score > best_score or \
                cfg.DATA.TASK_TYPE == 'regression' and val_score < best_score:   ## 添加 and val_acc_score > best_acc_score and val_pre_score > best_pre_score
            best_score, best_epoch = val_score,  epoch  ## 添加 best_acc_score, best_pre_score,和 val_acc_score, val_pre_score,
            save_best_checkpoint(cfg, epoch, model, best_score, best_epoch, optimizer, lr_scheduler, logger)  ## 添加 best_acc_score, best_pre_score,
            early_stop_cnt = 0
        else:
            early_stop_cnt += 1
        # 6: Early stopping.
