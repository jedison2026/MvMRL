...
from model import MvMRL  ## source.
...
def cross_validate(cfg, logger):
    ...
    for fold_num, scores in enumerate(all_scores):
        logger.info(f'Seed {init_seed + fold_num} ==> test {cfg.DATA.METRIC} = {np.nanmean(scores):.5f}')
        if cfg.SHOW_EACH_SCORES:
            for task_name, score in zip(task_names, scores):
                logger.info(f'Seed {init_seed + fold_num} ==> test {task_name} {cfg.DATA.METRIC} = {score:.5f}')

    # Report scores across models
    avg_scores = np.nanmean(all_scores, axis=1)
    mean_score, std_score = np.nanmean(avg_scores), np.nanstd(avg_scores)
    logger.info(f'Overall test {cfg.DATA.METRIC} = {mean_score:.5f} ± {std_score:.5f}')

    if cfg.SHOW_EACH_SCORES:
        for task_num, task_name in enumerate(task_names):
            logger.info(f'Overall test {task_name} {cfg.DATA.METRIC} = '
                        f'{np.nanmean(all_scores[:, task_num]):.5f} ± {np.nanstd(all_scores[:, task_num]):.5f}')
...
def hyperopt(cfg, logger):
    ...
    def objective(hyperparams):
        ...
        # Record results(这里是保存模型数据时用，所以前面的调用的train中的model是没有影响的)
        temp_model = MvMRL(out_dim=(len(hyper_cfg.DATA.TASK_NAME)) * 2)  ##, filter_num=32)   ### 这里改了！！！！！！！
